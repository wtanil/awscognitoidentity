...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (username && password) {
        ...
        [[self.devAuthClient login:username password:password] continueWithExecutor:[AWSExecutor mainThreadExecutor] withBlock:^id(AWSTask *task) {
            NSLog(@"alertView not null in login");
            if (task.cancelled) {
                NSLog(@"alertView not null in login cancel");
                [[AmazonClientManager errorAlert:@"Login canceled."] show];
            }
            else if (task.error) {
                NSLog(@"alertView not null in login error");
                ...
                NSLog(@"alertView not null in login go go");
